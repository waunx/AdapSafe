...
class AdapSafe:
    ...
    def train_cs(self, task_id=None, snap_buffer=None, train_tasks_buffer=None, adaptation_step=False):
        ...
        # 也就是说train task中一共sample了snap_buffer个经验再加上2 * len(neg_tasks_ids)个
    ...
    def update_prox_w_ess_factor(self, cs_model, x, beta=None):
        ...
        if np.isnan(ess_prox_factor) or np.isinf(
                ess_prox_factor) or ess_prox_factor <= self.r_eps:  # make sure that it is valid
    ...
    def do_training(self,
        ...
        for it in range(iterations):
            ...
            if len(src_task_ids) > 0:
                x, y, u, r, d, pu, pr, px, nu, nr, nx = replay_buffer.sample_tasks(task_ids=src_task_ids,
            ...
            pre_act = self.actor_target(next_obs, act_rew)

            if cbf:
                safe_act = pre_act
            else:
                safe_act = pre_act
            next_action = (safe_act + noise).clamp(-self.max_action, self.max_action)
            ...
            critic_loss_temp = F.mse_loss(current_Q1, target_Q, reduction='none') + F.mse_loss(current_Q2, target_Q,
                                                                                               reduction='none')
            ...
            # critic_loss = (critic_loss_temp).mean()
            ...
            if it % self.policy_freq == 0:
                ...
                # Compute actor loss
                actor_action = self.actor(obs, pre_act_rew)

                # actor_loss_temp = -1 * beta_score * self.critic.Q1(obs, self.actor(obs, pre_act_rew), pre_act_rew)
                actor_loss_temp = -1 * beta_score * self.critic.Q1(obs, actor_action, pre_act_rew)
    ...
    def train_TD3(
        ...
        for it in range(iterations):
            ...
            # Delayed policy updates
            ########
            # critic是每个train_steps都更新 但actor更新频率会慢一些（之前没见过）
            if it % self.policy_freq == 0:

                # Compute actor loss
                actor_action = self.actor(obs, pre_act_rew)
                actor_loss = -1 * self.critic.Q1(obs, actor_action, pre_act_rew).mean()
