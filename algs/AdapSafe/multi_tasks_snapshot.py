...
class MultiTasksSnapshot(object):
	...
	def sample(self, task_ids, batch_size):
		# sample仅仅是为了做context的输入 所以只提取出想要的历史信息和当前状态
		'''
			Returns tuples of (state, next_state, action, reward, done,
							  previous_action, previous_reward, previous_state
							  )
		'''
		# 对于每个task_id都从对应的task buffer中sample batch个经验
		if len(task_ids) == 1:
			# pu: (hist, action_dim) 这里没有flatten
	...
	def sample_tasks(self, task_ids, batch_size):
		'''
			Returns tuples of (state, next_state, action, reward, done, previous_action, previous_reward, previous_state)
		'''
		mb_xx = []
		mb_yy = []
		mb_u = []
